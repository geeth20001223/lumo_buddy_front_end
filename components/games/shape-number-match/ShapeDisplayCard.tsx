...
export function ShapeDisplayCard({ mode, emoji, count }: ShapeDisplayCardProps) {
  ...
  return (
    ...
    >
      <div className="min-h-[140px] rounded-[2rem] border border-white/80 bg-white/40 p-3.5 shadow-premium backdrop-blur-xl sm:min-h-[170px] sm:p-5 flex flex-col items-center justify-center space-y-3.5">
        {mode === "COUNT_TO_NUMBER" ? (
          <div className={getGridLayout()}>
            {Array.from({ length: count }).map((_, i) => (
              <motion.span
                ...
                className="select-none text-3xl drop-shadow-sm sm:text-5xl"
              >
                {emoji}
              </motion.span>
            ))}
          ...
          <div className="text-center space-y-2">
            ...
            <motion.div
              ...
              className="text-5xl font-black text-slate-900 drop-shadow-sm sm:text-6xl"
            >
              {count}
            </motion.div>
          </div>
        ...
        <div className="pt-1 text-center sm:pt-2">
          <p className="text-sm font-bold italic tracking-tight text-slate-500 sm:text-base">
